				...
				for (let t = 0; t < typeList.length; t++) {
					for (let ta = 0; ta < typeList[t].abilities.length; ta++) {
						if (abilityList[a].name == typeList[t].abilities[ta]) {
							ab += "<li class='og-tag-primary'>Type (" + typeList[t].name + ")</li>";
						}
					}
				}
				// ability types
					...
					for (ft = 0; ft < focusList[f].abilities.length; ft++) {
						for (let fa = 0; fa < focusList[f].abilities[ft].length; fa++) {
							if (abilityList[a].name == focusList[f].abilities[ft][fa].name) {
								ab += "<li class='og-tag-primary'>Focus (" + focusList[f].name + ", Tier " + (ft + 1) + ")</li>";
							}
						}
					}
				}
					...
					if (genreList[g].abilitiesorigin != undefined) {
						for (ga = 0; ga < genreList[g].abilitiesorigin.length; ga++) {
							if (abilityList[a].name == genreList[g].abilitiesorigin[ga]) {
								ab += "<li class='og-tag-primary'>Superhero Origin Ability</li>";
							}
						}
					}
					if (genreList[g].abilitiesmid != undefined) {
						for (ga = 0; ga < genreList[g].abilitiesmid.length; ga++) {
							if (abilityList[a].name == genreList[g].abilitiesmid[ga]) {
								ab += "<li class='og-tag-primary'>Mid-Tier Ability (" + genreList[g].genre + ")</li>";
							}
						}
					}
					if (genreList[g].abilitieshigh != undefined) {
						for (ga = 0; ga < genreList[g].abilitieshigh.length; ga++) {
							if (abilityList[a].name == genreList[g].abilitieshigh[ga]) {
								ab += "<li class='og-tag-primary'>High-Tier Ability (" + genreList[g].genre + ")</li>";
							}
						}
					}
				}
				if (abilityList[a].dupminor == true) {
					ab += "<li class='og-tag-low'>Minor Differences</li>";
				}
				else if (abilityList[a].dupmajor == true) {
					ab += "<li class='og-tag-danger'>Major Differences</li>";
				}
...
function getSection(ch) {
	...
	if (sectionList[ch].name == "Introduction") { 
		...
		for (let g = 0; g < genreList.length; g++) {
			...
			if (genreList[g].type != undefined) { 
				...
				if (genreList[g].genre == "Superheroes" && genreList[g].subgenre == undefined) {
					...
					alert += getBody(["You can adapt or reskin <a href='#define-type'>types</a> to superhero by consulting the table and making the following changes:", ["Replace bonuses to Might, Speed, and Intellect <a href='#define-stat'>Pool</a> points with new totals and maximums (determined by rank).", "The <a href='#define-ability-superheroics'>Superheroics</a> ability.", "A number of additional points to the <A href='#define-stat'>Pool</a> you choose for the <a href='#define-ability-superheroics'>Superheroics</a> ability (determined by rank).", "A number of <a href='#define-power-shift'>power shifts</a> (determined by rank).", ]]);
		...
		chx += getBody(["Abilities are noted by their source&mdash;for example, a <a href='#define-type'>type</a>, a <a href='#define-focus'>focus</a> and <a href='#define-tier'>tier</a>, or an ability list associated with a <a href='#define-genre'>genre</a>.", ["<strong>Minor Differences:</strong> These abilities have several entries in the <a href='https://www.montecookgames.com/store/product/cypher-corebooks/'>Cypher Character Rulebook</a> with different phrasing, but identical mechanics. The OG-CRD condenses these to a single entry, so you may notice small discrepancies between the two sources.", "<strong>Major Differences:</strong> These abilities share a name, but their exact mechanics differ depending on the source of the ability."]]);
		// quick ref
