...
const skillList = [
	...
	{ name: "Charm", ref: ["CCR 20"], description: "You know how to interact with people to get them to react to you positively. In a disagreement, you're adept at getting them to agree with your position.", sidebar: "<p>Any truthful positive social interaction is covered by charm.</p>", }, 
	...
	{ name: "Deception", ref: ["CCR 21"], description: "You are a capable liar, able to use speech and the written word to deceive and misinform others.", sidebar: "<p>Any untruthful positive social interaction is covered by deception.</p>", }, 
	...
	{ name: "Intimidation", ref: ["CCR 22"], description: "You're good at swaying others with threatening words and/or a fearsome presence.", sidebar: "<p>Any negative social interaction is covered by intimidation.</p>", }, 
	...
	{ name: "Mechanics", ref: ["CCR 22"], description: "You can diagnose, repair, maintain, and (with the right tools and parts) build engines and various machines appropriate to your game's setting.", sidebar: "<p>Mechanics training often requires resources to build and repair engines.</p>", }, 
...
abilities: [
		...
		[
			...
			{ name: "Shapechanger Healing", req: ["Beast Communication", "Soothe the Savagem",], }, 
		], 
		[
...
{ name: "Hunts", ref: ["CCR 194&ndash;195",], genres: [ {name: "Fantasy", theme: "Assassin, Ranger", }, {name: "Real World", theme: "Bounty Hunter", }, {name: "Science Ficion", theme: "Bounty Hunter", }, {name: "Superheroes", theme: "Bounty Hunter", }, ], 
...
abilities: [
		...
		[
			...
			{ name: "Font of Recovery", req: ["Aleviate Malady",], }, 
		], 
		[
...
const abilityList = [
	...
	{ name: "Beast Healing", ref: ["CCR 192",], effects: ["When you change into your beast form, you are immediately healed of one wound ofany severity. This ability renews after you takea ten-hour recovery. Enabler."], }, 
	...
	{ name: "Fury", ref: ["CCR 83",], cost: 3, pool: "Might", effects: ["Your melee attacks inflict+2 damage. This ability lasts until you use aten-minute or longer recovery. Action.", ], }, 
	...
	{ name: "Psychic Burst", ref: ["CCR 162",], cost: 3, plus: true, pool: "Intellect", effects: ["You blast waves of mental force into the minds of three targets within short range (make an Intellect roll against each target). This burst inflicts 3 Intellect damage (ignores armor).Telepathic<em>Effort:</em> Attack an additional target.TelepathicAction."], }, 
	...
	{ name: "Retribution", ref: ["CCR 67",], cost: 1, pool: "Intellect", effects: ["You call forth the powers of a deity to assail a foe within short range that you can see. If your Intellect attack hits, the target takes3 damage; they are also marked with an obvious signrelating to that deity until you take a ten-minute or longer recovery. For example, if you call for the retribution of Thesk the Serpent Lord, the attack looks like ephemeral snakes twining around and biting your foe, and they are marked on their forehead or another obvious location with glowing fangs or a snake's head.", "Last action.", "<em>At tier 3:</em> The retribution increases to 5 damage.", ], }, 
	...
	{ name: "Talons or Claws", ref: ["CCR 191",], cost: 1, pool: "Might", effects: ["You can make your fingertips grow sharp talons or claws (whichever matches your beast form) until you use aten-minute or longer recovery, during which time your unarmed attacks deal an additional 1 damage. Action when in your normal form; enabler when using Beast Form."], }, 
...
{ name: "Frost Wall", ref: ["CCR 271",], manifest: true, pow: ["Med"], effect: ["You create a wall of swirling frost up to a short distance away, lasting until you use aten-minute or longer recovery. The wall is up to 30 feet by 30 feet by 1 foot (9 m by 9 m by 30 cm) and it conforms to the space available. Anything passing through it takes 3 damage.", ], genreitems: [["frost diamond", "Fantasy", ], ["cryoplane module", "Science Fiction", ], ], }, 
...
{ name: "Image Projector", ref: ["CCR 272",], manifest: true, pow: ["Low"], effect: ["Projects a specific illusory or holographic scene in an immediate area up to a short distance away from you. The scene remains in place after it is created, and it includes appropriate movement, sound, and scent. For example, the scene might contain a ferocious wolf that paces back and forth but doesn't leave the area. The scene lasts until you use aone-hour or longer recovery.", "Roll to determine the image.", ], genreitems: [["illusion gem", "Fantasy", ], ["hologram projector", "Science Fiction", ], ], }, 
...
{ name: "Sustenance", ref: ["CCR 279",], manifest: true, pow: ["Low"], effect: ["For oneday you can go without food and water without any ill effects, as if you've been eating and drinking healthily during this time.", ], genreitems: [["amulet of sustenance", "Fantasy", ], ["nutrition recycler", "Science Fiction", ], ], }, 
...
const genreList = [
	...
	{ 
		...
		ref: ["CCR 32",], 
		summary: ["In a crime thriller game, you'll likely find yourself trying to solve one or more crimes&mdash;possibly a series of connected crimes&mdash;especially murder. For example:", ["You're a detective, forensic specialist, journalist, true crime podcaster, author, police consultant, or a private investigator.", "You're part of a criminal gang, the mob, or another group on the wrong side of the law.", ], "Choose skills to choose include those useful in crime scene investigation, interviewin witnesses and suspects, finding clues through keen perception, gathering information with a wide net, analyzing evidence, and putting the pieces together.", ], 
	...
	{ 
		...
		subgenre: "Historical", 
		ref: ["CCR 35",], 
		summary: ["History provides endless settings across human existence and culture&mdash;for example, World War II, the Renaissance, the 1930s, ancient Greece, feudal Japan, ancient Egypt, a Mayan city-state.", "The GM should provide you with information on appropriate characters&mdash;for example, if the game is set in medieval Europe, you mightbe a knight, courtier, noble, minstrel, or scofflaw. If the game is set in ancient Greece, maybe you're an artisan, merchant, soldier (sometimes called a hoplite), scholar, philosopher, or athlete of the era.", "The game might begin with a major historical incident to give you the thrill of \"being there\".", ], 
	...
	{ 
		...
		summary: ["A dungeon fantasy game usualy includes challenges presented bt a host of bizarre monsters and life-draining undead, traps, curses, and&mdash;eventually&mdash;dragons or gods. PCs explore perilous places&mdash;for example, dungeons, caverns, and ancient ruins. But the rewards include treasures of gold, gems, jewelry&mdash;and best of all&mdash;magic.", ], 
		species: ["Choose or create a <a href='#define-descriptor'>descriptor</a>, and pair it with  one of the species avalable in the setting.", "Unless the GM says otherwise, PCs can't see in complete darkness&mdash;regardless of species."], 
	...
	}, 
	{
		...
		summary: ["If you're playing in a space opera game, you can expect faster-than-light (FTL) ships, and technology so advanced it may as well be magic. This makes sense: space operas incorporate elements and themes from <a href='#define-genre-fantasy'>fantasy</a>. Some space operas place a premium on a feeling of real-world versimilitude, while \"pulp\" settings can veer into surreal.", "Travel times might not be important, psychic abilities might be prevalent, and planets might be covered in a single biome. Alien creatures might be monsters, or almost suspiciously similar to humans. An entire space opera could occur inside a megastructure orbiting the event horizon of <em>Sagittarius A*</em>&mdash;the supermassive black hole at the center of the Milky Way. Space operas can also explore the struggle against the underlying rules of reality itself.", ], 
		...
		skillsoptions: ["Astronomy", "Athletics", "Attacking", "Biology", "Charm", "Chemistry", "Crafting", "Cybernetics", "Deception", "Defending", "Disguise", "Driving", "Engineering", "Escaping", "Forensics", "Gathering information", "Geology", "Gunnery", "Gymnastics", "Hacking", "Healing", "Heavy equipment operation", "History", "Identifying", "Initiative", "Intimidation", "Lockpicking", "Magic lore", "Mathematics", "Mining", "Navigation", "Outdoor survival", "Perception", "Performance", "Philosophy", "Physics", "Pickpocketing", "Piloting", "Piloting spacecraft", "Psychic lore", "Psychology", "Publishing", "Recognizing motive", "Religious lore", "Riding", "Scavenging", "Stealth", "Systems operation", "Tracking", "Zero-G fluency", ], 
		species: ["Choose or create a <a href='#define-descriptor'>descriptor</a>, and pair it with one of the species avalable in the setting. If you'd like to play a \"near-human\" alien, choose the <a href='#define-species-human'>Human</a> descriptor and work out any interesting biological quirks about your species with the GM.", "In some cases, a species might so advanced or fundamentally <em>different</em> that your choice of <a href='#define-type'>type</a> or <a href='#define-focus'>focus</a> is a better way of addressing species&mdash;for example, the <a href='#define-type-android'>Android</a> type."], 
	...
	}, 
	{
		...
		focus: ["Postapocalypse games doen't usually include foci based on magic or science fiction elements.", ], 
		...
		wounds: ["Postapocalypte is a <strong>realistic</strong> genre.", ], 
		manifestcyphers: ["Usually none.", ], 
	}, 
	{
		genre: "Superheroes", 
...
function createTable(table) {
	for (let t = 0; t < tableList.length; t++)  {
		if (tableList[t].name == table) {
			...
			tc += "<caption class='visually-hidden'>" + table + " Abilities</caption>";
			tc += "<thead>";
			// create columns
	...
	if (sectionList[ch].name == "Introduction") { 
		...
		for (let char = 0; char < characteristicList.length; char++) {
			...
			if (characteristicList[char].dt == "Core Character") {
				...
				def += "<p class='og-hang'><strong>Background Skill:</strong> One of your two <a href='#define-skill'>genre skills</a> is your background skill. Use it as a seed to create a revealing narrative about your past&mdash;or come up with your background first, then select a relevent skill.</p>";
				def += "<p class='og-hang'><strong>Character Sentence:</strong> The character senstence is <em>\"I am an adjective nound who verbs\"</em>. The adjective is your <a href='#define-descriptor'>descriptor</a>&mdash;and maybe your <a href='#define-species'>species</a>. The noun is your <a href='#define-type'>type</a>). The verb is your <a href='#define-focus'>focus</a>. Your available choices are informed by the game's genre.</p>";
				chx += createAlert(createDef("core-character", def));
			}
			else {
				def = createHeader(3, createID(characteristicList[char].dt), characteristicList[char].dt, "og-border");
				...
				if (characteristicList[char].dt == "Skill") {
					...
					def = createHeader(4, "weapons", "Weapons", "");
					def += createRef(["CCR 13", "CCR 238", "CCR 304",]);
					def += getBody(["If you <a href='#define-attack'>attack</a> using a weapon you can't <a href='#define-skill'>freely use</a>, the attack roll is hindered.", ["<strong>Melee:</strong> You can use a melee weapon to attack a foe within immediate range.", "<strong>Ranged:</strong> Ranged weapons specify a maximum range. If a target is beyond the weapon's range, your attack is hindered.", "<strong>Explosive Weapons:</strong> Explosions are an area attack that usually affects an immediate area."]]);
					def += createTable("Weapons");
					chx += createDef("weapon", def);
					def = createHeader(4, "armor", "Armor", "");
		...
		for (let g = 0; g < genreList.length; g++) {
			...
			if (genreList[g].equipment != undefined) { 
				...
				if (genreList[g].equipmenttable != undefined) {
					...
					if (genreList[g].equipmenttable == "Science Fiction") { chx += "<p class='small og-hang'>&dagger; &mdash; <em>fantastic technology only avaiable in far-future or advanced-technology settings</em></p>"; }
					chx += "<div class='accordion''>"; // open accordion
					for (let t = 0; t < tableList.length; t++) {
		...
		def += getBody(["Difficulty and determine your chance to succeed a <a href='#define-task'>task</a>. The target number when <a href='#define-roll'>rolling the die</a> is difficulty &times; 3.", ["<strong>Level:</strong> NPCs, objects, and effects have a level, which is the difficulty of <a href='#define-task'>tasks</a> made in opposition to them.", "<strong>Easing:</strong> You can lower the difficulty applying relevant <a href='#define-skill'>skills</a>, <a href='#define-asset'>assets</a>, and <a href='#define-effort'>Effort</a>, or by having favorable circumstances.", "<strong>Hindering:</strong> Unfavorable circumstances or an <a href='#define-skill'>inability</a> raise difficuty."]]);
		...
		def += getBody(["You roll a twenty-sided die&mdash;A d20&mdash;to determine whether an action is successful. When the text refers to \"roll,\" in the generic, it's a d20 roll.", ["<strong>Retrying:</strong> If you fail and attempt a <a href='#define-task'>task</a> again, you must apply <a href='#define-effort'>Effort</a>, unless you're in a dynamic situation like <a href='#define-attack'>attacking</a> in combat.", "<strong>Special Rolls:</strong> In addiion to determining success and failure, special roll results produce additional positive or helpful effects. If you roll a 17, the attack must succeed to deal the additional damage."]]);
		...
		def += getBody([["<strong>Wounds:</strong> Most creatures, NPCs, and physical hazards inflict <a href='#define-wound'>wounds</a> of a severity listed in the attacker's statistics or determined by the GM.", "<strong>Damage to Pools:</strong> Special attacks can reduce your <a href='#define-stat'>Pools</a>&mdash;for example, a a numbing poison that inflicts <a href='#define-stat'>Speed</a> damage. If damage reduces a stat Pool to 0, any further damage to that stat (including any excess damage from the triggering attack) become <a href='#define-wound'>wounds</a>.", "<strong>Special Damage:</strong> Effects that linger for a round or more&mdash;for example, paralysis.", "<strong>Non-Rest Recovery:</strong> Ending some effects requires you to use one or more <a href='#define-recovery'>recoveries</a> without gaining any of their benefits, or a successful <a href='#define-defend'>defense roll</a>."]]);
