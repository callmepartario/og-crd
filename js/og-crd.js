		...
		for (let char = 0; char < characteristicList.length; char++) {
			...
			if (characteristicList[char].dt == "Core Character") {
				...
				// build types
					...
					chx += createEdnote("The OG-CRD omits type characteristics that reiterate <a href='#define-core-character'>core character</a> statistics and benefits assigned by <a href='#define-genre'>genre</a>, allowing GMs to easily adapt types to other genres. For additional discrepancies with the <a href='https://www.montecookgames.com/store/product/cypher-corebooks/'>Cypher Character Rulebook</a>, see the <a href='#ability-index'>Ability Index</a>.");
					for (let t = 0; t < typeList.length; t++) {
						let type = "";
						...
						if (typeList[t].equipment != undefined) {
							...
							chx += "<h5 class='accordion-header' id='" + createID(typeList[t].name + "equipment") + "'>";
							chx += "<button class='accordion-button collapsed text-uppercase p-2' type='button' data-bs-toggle='collapse' data-bs-target='#" + createID(typeList[t].name + "equipment") + "-collapse' aria-expanded='false' aria-controls='" + createID(typeList[t].name + "equipment") + "-collapse'>" + typeList[t].name + " Equipment Bundle </button>";
							chx += "</h5>";
							chx += "<div id='" + createID(typeList[t].name + "equipment") + "-collapse' class='accordion-collapse collapse' aria-labelledby='" + createID(typeList[t].name + "equipment") + "'>"; // open collapse
							chx += "<div class='accordion-body p-2 pt-3 og-stripe'>"; // open body
							chx += getBody([typeList[t].equipment]);
							chx += "</div>"; // close body
